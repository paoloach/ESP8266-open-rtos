...
fun getSerialPort(project: Project):SerialPortData?{
    ...
    val port =  config["ESPTOOLPY_PORT"]?.let { it } ?: setting.serialPort
    val baud = config["ESPTOOLPY_BAUD"]?.toIntOrNull() ?:setting.baud


    val portNames = SerialPortList.getPortNames()
    if (portNames.isEmpty())
        return SerialPortData(port, baud)
    SerialPortList.getPortNames().firstOrNull{it == port}?.let { return  SerialPortData(port, baud)}
